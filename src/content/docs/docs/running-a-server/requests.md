...
4. Check that the intended users' [access group](/docs/running-a-server/access) allows media requests.
...
Install the required [plugin](/docs/running-a-server/plugins) first if it is missing. Test with one approved request and check both Silo and the external service. Do not assume approval alone confirms that the service accepted or downloaded the request.

Configure [notifications](/docs/running-a-server/notifications) if users should receive delivery updates.
