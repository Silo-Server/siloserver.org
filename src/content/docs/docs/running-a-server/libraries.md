...
If files are found but identified incorrectly, use [Match Item](/docs/running-a-server/metadata#correct-a-wrong-match). Repeated scans do not fix an ambiguous name.
...
For ongoing imports from another service, set up [Autoscan](/docs/running-a-server/autoscan) after a normal scan succeeds.
